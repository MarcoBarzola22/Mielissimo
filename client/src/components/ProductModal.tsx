...
export const ProductModal = ({ producto, isOpen, onClose }: ProductModalProps) => {
  ...
  const [varianteSeleccionada, setVarianteSeleccionada] = useState<ProductVariant | null>(null);
  ...
  const precioFinal = varianteSeleccionada
    ? precioBase + Number(varianteSeleccionada.precio_extra)
    : precioBase;

  const handleAddToCart = () => {
    if (variants.length > 0 && !varianteSeleccionada) {
      ...
      <DialogContent className="sm:max-w-[425px] bg-white rounded-xl">
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="w-full h-48 bg-gray-100 rounded-md overflow-hidden flex items-center justify-center">
            <img
              src={producto.imagen ? `http://localhost:3000/uploads/${producto.imagen}` : '/placeholder.png'}
              alt={producto.nombre}
              ...
              <div className="flex flex-wrap gap-2">
                {variants.map((v, idx) => (
                  <button
                    ...
                    className={`px-3 py-1 text-sm rounded-full border transition-all ${varianteSeleccionada === v
